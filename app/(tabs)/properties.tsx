...
export default function PropertiesScreen() {
  ...
  const averageOccupancy =
    mockProperties.reduce((sum, property) => sum + property.occupancyRate, 0) /
    mockProperties.length;
  const totalIncome = mockProperties.reduce(
    (sum, property) => sum + property.monthlyIncome,
    0
  );
  const yearlyTotalIncome = mockProperties.reduce(
    (sum, property) => sum + property.monthlyIncome * 12,
    0
  );
  ...
  const renderPropertyCard = ({ item }: { item: Property }) => (
    <TouchableOpacity
      ...
      style={{ elevation: 3 }}
    >
      <Image source={item.imageUrl} className="w-full h-65" />
      <View className="p-4">
        <Text
      ...
      <ScrollView className="flex-1">
        <View className="px-4 pt-2 pb-4">
          ...
          >
            ...
            {/* Units Stat */}
            <View
              className={`flex-1 mx-2 p-4 rounded-xl ${
                isDark ? "bg-gray-800" : "bg-white"
              }`}
              style={{ elevation: 2 }}
            >
